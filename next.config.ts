...
console.log(` 🧠 \x1b[1mEgregore\x1b[0m v${process.env.NEXT_PUBLIC_BUILD_PKGVER} (@${process.env.NEXT_PUBLIC_BUILD_HASH})`);

// Non-default build types
const buildType =
  process.env.Dapp_AGI_BUILD === 'standalone' ? 'standalone' as const
    : process.env.Dapp_AGI_BUILD === 'static' ? 'export' as const
      : undefined;

buildType && console.log(` 🧠 Egregore: building for ${buildType}...\n`);
...
if (process.env.POSTHOG_API_KEY && process.env.POSTHOG_ENV_ID) {
  console.log(' 🧠 \x1b[1mEgregore\x1b[0m: building with PostHog error tracking and source maps...');
